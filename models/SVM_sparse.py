...
def classify(csv_path, n_repeats=50):
    """
    Sparse-peak SVM (RBF) inference with latency benchmarking.
    Assumes SVC(probability=True) was used during training.
    """
    # 0) Load pipeline ONCE
    pipe = joblib.load("ftir_model_sparse_svm.pkl")
    pca   = pipe["pca"]
    model = pipe["model"]
    wav   = pipe["wav"]
    cols  = pipe["columns"]
    ...
    for _ in range(n_repeats):
        ...
        # 3) PCA + SVM prediction (probabilities)
    ...
    print(f"\n🔬 Predicted compound (SVM, sparse): {last_pred}\n")
    ...
    plt.title("Prediction Probabilities (SVM, sparse features)")
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python SVM_sparse.py <path_to_spectrum.csv>")
        sys.exit(1)
    classify(sys.argv[1])
