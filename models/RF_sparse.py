...
def classify(csv_path, n_repeats=50):
    """
    Sparse-peak Random Forest inference with latency benchmarking.
    """
    # 0) Load pipeline ONCE
    pipe = joblib.load("ftir_model_sparse_rf.pkl")
    pca   = pipe["pca"]
    model = pipe["model"]
    wav   = pipe["wav"]
    cols  = pipe["columns"]
    ...
    for _ in range(n_repeats):
        ...
        # 3) PCA + RF prediction (probabilities)
    ...
    print(f"\n🔬 Predicted compound (RF, sparse): {last_pred}\n")
    ...
    plt.title("Prediction Probabilities (Random Forest, sparse features)")
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python RF_sparse.py <path_to_spectrum.csv>")
        sys.exit(1)
    classify(sys.argv[1])
